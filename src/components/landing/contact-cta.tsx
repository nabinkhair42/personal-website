...
export function ContactCTA() {
  return (
    <section className={`${SECTION.base} ${BG.surface}`}>
      ...
      <div className={`${SECTION.containerNarrow} text-center`}>
        {/* Section Header */}
        <div className={`mb-12 ${ANIM.in}`}>
          <div className="flex items-center justify-center gap-4 mb-6">
            <div className="w-8 h-px bg-zinc-300 dark:bg-zinc-700"></div>
            <span className={TYPO.sectionKicker}>Let&apos;s Connect</span>
            <div className="w-8 h-px bg-zinc-300 dark:bg-zinc-700"></div>
          </div>
          ...
          <p className={`${TYPO.paragraph} max-w-2xl mx-auto mb-12`}>
            I&apos;m currently available for freelance projects and full-time
            opportunities. If you&apos;re interested in working together or have
            an exciting project in mind, let&apos;s have a conversation.
          </p>
        </div>

        {/* CTA Buttons */}
        <div
          className={`flex flex-col sm:flex-row sm:justify-center gap-4 ${ANIM.in} ${ANIM.delay(600)}`}
        >
          <Link href="/hire-me">
        ...
        {/* Contact Methods */}
        <div
          className={`grid grid-cols-1 md:grid-cols-2 gap-6 mt-16 ${ANIM.in} ${ANIM.delay(500)}`}
        >
