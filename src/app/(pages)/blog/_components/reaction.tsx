...
export default function ReactionButton({
  ...
  const clientCache = useRef<{
    [key: string]: {
      data: { reactions?: { [key: string]: number } };
      timestamp: number;
    };
  }>({});
  ...
  const getEmojiInfo = (emoji: string) => {
    return (
      EMOJI_LIST.find((item) => item.emoji === emoji) || {
        emoji,
        label: emoji,
  ...
  return (
    <Card
      className={cn(
        'w-full max-w-2xl mx-auto border  border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 rounded-none border-none shadow-none',
        className
      )}
    >
      ...
      <CardContent className="pb-6">
        {error && (
          <Alert
            variant="destructive"
            className="mb-4 border-red-200 dark:border-red-800"
          >
        ...
        {loading ? (
          <div className="flex flex-wrap gap-3 justify-center">
            {EMOJI_LIST.map((_, index) => (
              <Skeleton
                key={index}
                className="h-12 w-20 bg-zinc-200 dark:bg-zinc-800"
              />
            ))}
          </div>
        ) : (
        ...
        <CardFooter className="pt-0 pb-6 flex flex-col sm:flex-row items-center justify-center gap-4 text-sm text-zinc-600 dark:text-zinc-400">
          <HoverCard>
            ...
            <HoverCardContent className="w-80 p-4 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
              <div className="space-y-3">
                <h4 className="text-sm font-light text-zinc-900 dark:text-zinc-100">
                  Reaction Summary
                </h4>
                  ...
                  {EMOJI_LIST.map(({ emoji, label }) => {
                    ...
                    return (
                      <div key={emoji} className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-mono text-zinc-600 dark:text-zinc-400 min-w-[60px]">
                          {label}
                        </span>
                        <div className="flex-1 h-2 bg-zinc-200 dark:bg-zinc-800">
                          <div
                            ...
                            style={{
                              width: `${Math.max(10, (count / totalReactions) * 100)}%`,
                            }}
                          />
                        </div>
          ...
          {topReaction && (
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant="outline"
                className="border-zinc-300 dark:border-zinc-700 font-mono text-xs"
              >
